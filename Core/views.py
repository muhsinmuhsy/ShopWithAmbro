...
#     if cart:
#         sub_total_price = sum(item.product.price * item.quantity for item in cart)
...
#     # calculation subtotal without tax
#     if cart:
#         sub_total_price = sum(item.product.price * item.quantity for item in cart.cart_set.all())
#         cart_items = Cart.objects.filter(cart=cart)
#     else:
#         sub_total_price = 0
#         cart_items = None
...
#         try:
#             cart = Cart.objects.get(user_session_key=user_session_key)
#             cart_quantity_total = sum(item.quantity for item in cart.cart_set.filter(ordered=False))
